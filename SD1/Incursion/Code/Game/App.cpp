...
float g_totalElapsedSecondsSinceStart;
...
App::App()
	...
	float pad=0.f;
	g_theWorldBounds=new AABB2(pad,pad,g_screenWidth-pad,g_screenHeight-pad);
	m_game = new Game();
	g_doShowDebugGraphics=false;
	g_totalElapsedSecondsSinceStart=0.f;
	}
...
void App::Update(float deltaSeconds)
	{
	g_totalElapsedSecondsSinceStart+=deltaSeconds;
	g_theAudio->update();
	if (g_theInput)
...
void App::RunFrame()
	...
	const double MAXIMUM_DELTA_SECONDS = 0.1;  // one frame per second
	double deltaSeconds=timeNow-timeOfLastRunFrame;
	deltaSeconds=minOf(deltaSeconds,MAXIMUM_DELTA_SECONDS);
