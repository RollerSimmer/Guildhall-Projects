...
Entity::Entity(int indexIntoGameArray, EntityType mytype, const Vector2& pos,float radius,float speed) 
	...
	, m_isMoveable(true)
	, m_canOverlap(false)
	, m_isInvincibleModeActive(false)
	, m_isForeverInvincible(false)
    , m_invincibilityCountdownSeconds(0.f)
	{
	m_angularVelocity = 0.f;
...
bool Entity::didJustDie() const
	...
	while (result && m_type==ENTITY_TYPE_PLAYER_TANK)
		break;
	return result;
	}
...
void Entity::toggleEndlessInvincibility()
	{
	m_isInvincibleModeActive=!m_isInvincibleModeActive;
	m_isForeverInvincible=true;
	}
...
void Entity::drawBaseSprite() const
	...
	if (m_baseTexture)
		...
		g_theRenderer->drawTexturedAABB2(bounds, *m_baseTexture, Vector2(0.f, 1.f), Vector2(1.f, 0.f), color,
...
void Entity::updateEnd(float /*deltaSeconds*/)
	...
	if (didJustDie() && m_shouldExplodeOnDeath)
		{
		while(m_type==ENTITY_TYPE_PLAYER_TANK)
			break;
...
void Entity::startTemporaryInvincibility(float durationSeconds)
	{
	m_isInvincibleModeActive=true;
	m_isForeverInvincible=false;
	m_invincibilityCountdownSeconds=durationSeconds;
	}
...
void Entity::updateInvincibility(float deltaSeconds)
	{
	if (m_isInvincibleModeActive)
		{
		if (!m_isForeverInvincible)
			{
			m_invincibilityCountdownSeconds-=deltaSeconds;
			if (m_invincibilityCountdownSeconds<=0.f)
				{
				m_invincibilityCountdownSeconds=0;
				m_isInvincibleModeActive=false;
				}
			}
		}
