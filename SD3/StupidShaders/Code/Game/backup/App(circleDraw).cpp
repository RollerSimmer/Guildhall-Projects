...
void App::initRendering()
	...
	int amtTrianglesToDraw=g_amtVerticesToDraw/3;

	float angleStep=360.f/(float)amtTrianglesToDraw;
	float radius=0.75f;

	for(int i=0;i<amtTrianglesToDraw;i++)
		{
		float angle=(float)i*angleStep;
		float angleNext=angle+angleStep;
		float x=radius*cosDegrees(angle);
		float y=radius*sinDegrees(angle);
		float xNext=radius*cosDegrees(angleNext);
		float yNext=radius*sinDegrees(angleNext);
		vertices[i*3+0]=Vertex3(Vector3(0, 0, 1), Rgba::WHITE, Vector2(-0.5f, 0.f-0.5f));
		vertices[i*3+2]=Vertex3(Vector3(x, y, 1), Rgba::WHITE, Vector2(x*0.5f-0.5f, -y*0.5f-0.5f));
		vertices[i*3+1]=Vertex3(Vector3(xNext, yNext, 1), Rgba::WHITE, Vector2(xNext*0.5f-0.5f, -yNext*0.5f-0.5f));
		}
